...
returned value will be a string containing only one character (for example, "b"). The first character has position zero, which causes the last one
...
let bCount = 0, inputString = stringInput;

for(let count = 0; count < stringInput.length ; count++){
...
function countChar(stringInput, charToCount){
	...
	let charCount = 0, inputString = stringInput;

	for(let count = 0; count < stringInput.length ; count++){

		if(inputString.charAt(count) === charToCount){
...
console.log(countChar('BBBbcccbbbCBccCCCCCCCCCCCC','c'));